...
import name.trifon.beeserver.config.KafkaProperties;
...
class BeeServerKafkaResourceIT {
    ...
    void setup() {
        ...
        BeeServerKafkaResource kafkaResource = new BeeServerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/bee-server-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/bee-server-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
