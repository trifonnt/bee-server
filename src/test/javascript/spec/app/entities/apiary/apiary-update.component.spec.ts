import { ComponentFixture, TestBed, fakeAsync, tick } from '@angular/core/testing';
import { HttpResponse } from '@angular/common/http';
import { FormBuilder } from '@angular/forms';
import { of } from 'rxjs';

import { BeeServerTestModule } from '../../../test.module';
import { ApiaryUpdateComponent } from 'app/entities/apiary/apiary-update.component';
import { ApiaryService } from 'app/entities/apiary/apiary.service';
import { Apiary } from 'app/shared/model/apiary.model';

describe('Component Tests', () => {
  describe('Apiary Management Update Component', () => {
    let comp: ApiaryUpdateComponent;
    let fixture: ComponentFixture<ApiaryUpdateComponent>;
    let service: ApiaryService;

    beforeEach(() => {
      TestBed.configureTestingModule({
        imports: [BeeServerTestModule],
        declarations: [ApiaryUpdateComponent],
        providers: [FormBuilder],
      })
        .overrideTemplate(ApiaryUpdateComponent, '')
        .compileComponents();

      fixture = TestBed.createComponent(ApiaryUpdateComponent);
      comp = fixture.componentInstance;
      service = fixture.debugElement.injector.get(ApiaryService);
    });

    describe('save', () => {
      it('Should call update service on save for existing entity', fakeAsync(() => {
        // GIVEN
        const entity = new Apiary(123);
        spyOn(service, 'update').and.returnValue(of(new HttpResponse({ body: entity })));
        comp.updateForm(entity);
        // WHEN
        comp.save();
        tick(); // simulate async

        // THEN
        expect(service.update).toHaveBeenCalledWith(entity);
        expect(comp.isSaving).toEqual(false);
      }));

      it('Should call create service on save for new entity', fakeAsync(() => {
        // GIVEN
        const entity = new Apiary();
        spyOn(service, 'create').and.returnValue(of(new HttpResponse({ body: entity })));
        comp.updateForm(entity);
        // WHEN
        comp.save();
        tick(); // simulate async

        // THEN
        expect(service.create).toHaveBeenCalledWith(entity);
        expect(comp.isSaving).toEqual(false);
      }));
    });
  });
});
